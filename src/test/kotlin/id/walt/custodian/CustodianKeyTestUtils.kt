...
object CustodianKeyTestUtils {
    ...
    fun StringSpec.standardKeyTests(custodian: Custodian) {
        ...
        "4: Delete keys" {
            val sizeBefore = custodian.listKeys().size

            custodian.deleteKey(key1.keyId.id)
            custodian.deleteKey(key2.keyId.id)

            custodian.listKeys().size shouldBeExactly (sizeBefore - 2)
        }
