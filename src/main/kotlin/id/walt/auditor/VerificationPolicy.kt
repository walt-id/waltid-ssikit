...
class VerificationPolicyResult {
    ...
    lateinit var errors: List<Throwable>

    private constructor(result: Boolean, errors: List<Throwable> = emptyList()){
        this.result = result
        this.errors = errors
    }
...
class SignaturePolicy : SimpleVerificationPolicy() {
    ...
    override fun doVerify(vc: VerifiableCredential) = runCatching {
        ...
        }.takeIf { it }?.let { VerificationPolicyResult.success() } ?: throw Exception("")
    }.getOrElse {
