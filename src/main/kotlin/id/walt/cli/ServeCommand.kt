...
import id.walt.rest.oidc.OidcAPI
...
class ServeCommand : CliktCommand(
    ...
    private val apiPort: Int by option(
        help = "Core API port [${CoreAPI.DEFAULT_CORE_API_PORT}]",
        names = arrayOf("-p", "--port")
    ).int().default(
        CoreAPI.DEFAULT_CORE_API_PORT
    )
    private val signatoryPort: Int by option(
        help = "Signatory API port [${SignatoryRestAPI.SIGNATORY_API_PORT}]",
        names = arrayOf("-s", "--signatory-port")
    ).int().default(
        SignatoryRestAPI.SIGNATORY_API_PORT
    )
    private val custodianPort: Int by option(
        help = "Custodian API port [${CustodianAPI.DEFAULT_Custodian_API_PORT}]",
        names = arrayOf("-c", "--custodian-port")
    ).int().default(
        CustodianAPI.DEFAULT_Custodian_API_PORT
    )
    private val auditorPort: Int by option(
        help = "Auditor API port [${AuditorRestAPI.AUDITOR_API_PORT}]",
        names = arrayOf("-a", "--auditor-port")
    ).int().default(AuditorRestAPI.AUDITOR_API_PORT)
    private val essifPort: Int by option(
        help = "Essif API port [${EssifAPI.DEFAULT_ESSIF_API_PORT}]",
    ...
    private val apiTargetUrls: List<String> by option(
        help = "Additional API target urls for swagger UI, defaults to root context '/'",
        names = arrayOf("-t", "--target-url")
    ).multiple()
