...
class ServeCommand : CliktCommand(
    ...
    help = """Run the walt.id SSI KIT as RESTful service.
        ...
        Runs the library as RESTful service and exposes following APIs on the specified bind address (default: ${RestAPI.DEFAULT_BIND_ADDRESS}):
         
         - walt.id Core API at the specified API port (default: ${RestAPI.DEFAULT_CORE_API_PORT})
                  
         - walt.id ESSIF API at the specified ESSIF API port (default: ${RestAPI.DEFAULT_ESSIF_API_PORT})
    ...
    private val apiPort: Int by option(help = "Core API port [${RestAPI.DEFAULT_CORE_API_PORT}]", names = arrayOf("-p", "--port")).int().default(RestAPI.DEFAULT_CORE_API_PORT)
    private val essifPort: Int by option(help = "Essif API port [${RestAPI.DEFAULT_ESSIF_API_PORT}]", names = arrayOf("-e", "--essif-port")).int().default(RestAPI.DEFAULT_ESSIF_API_PORT)
