...
object CustodianController {
    ...
    fun generateKey(ctx: Context) = ctx.json(custodian.generateKey(ctx.bodyAsClass<GenerateKeyRequest>().keyAlgorithm))
    ...
    fun getKey(ctx: Context) = ctx.json(custodian.getKey(ctx.pathParam("alias")))
    ...
    fun listKeys(ctx: Context) = ctx.json(ListKeyResponse(custodian.listKeys()))
    ...
    fun storeKey(ctx: Context) = custodian.storeKey(ctx.bodyAsClass<StoreKeyRequest>().key)

    /*@OpenApi(
        summary = "Deletes a specific key", operationId = "deleteKey",
        tags = ["Keys"], responses = [OpenApiResponse("200")]
    )*/
    fun deleteKey(ctx: Context) = custodian.deleteKey(ctx.pathParam("id"))
    ...
    fun getCredential(ctx: Context) = ctx.json(custodian.getCredential(ctx.pathParam("id")))
    ...
    fun listCredentials(ctx: Context) = ctx.json(ListCredentialsResponse(custodian.listCredentials()))
    ...
    fun listCredentialIds(ctx: Context) = ctx.json(ListCredentialIdsResponse(custodian.listCredentialIds()))
    ...
    fun storeCredential(ctx: Context) = ctx.bodyAsClass<StoreCredentialRequest>().run { custodian.storeCredential(alias, vc) }
    ...
    fun deleteCredential(ctx: Context) = custodian.deleteCredential(ctx.pathParam("alias"))
