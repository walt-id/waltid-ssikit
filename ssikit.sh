...
function extract() {
  ...
  if [[ ! -f build/distributions/waltid-ssi-kit-1.9.0.tar ]]; then
  ...
  (
    ...
    if tar xf waltid-ssi-kit-1.9.0.tar; then
...
function execute_debug() {
  if [[ -f build/distributions/waltid-ssi-kit-1.9.0/bin/waltid-ssi-kit ]]; then
    JAVA_OPTS="-Dorg.slf4j.simpleLogger.defaultLogLevel=DEBUG" build/distributions/waltid-ssi-kit-1.9.0/bin/waltid-ssi-kit "$@"
  else
...
function execute() {
  if [[ -f build/distributions/waltid-ssi-kit-1.9.0/bin/waltid-ssi-kit ]]; then
    build/distributions/waltid-ssi-kit-1.9.0/bin/waltid-ssi-kit "$@"
  else
    build_runscript_question
