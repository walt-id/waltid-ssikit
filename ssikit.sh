...
function extract() {
  ...
  if [[ ! -f build/distributions/waltid-ssi-kit-1.0.0.tar ]]; then
  ...
  (
    ...
    if tar xf waltid-ssi-kit-1.0.0.tar; then
...
function execute() {
  if [[ -f build/distributions/waltid-ssi-kit-1.0.0/bin/waltid-ssi-kit ]]; then
    build/distributions/waltid-ssi-kit-1.0.0/bin/waltid-ssi-kit "$@"
  else
    header
